import { Bell, Moon, Sun, PanelLeft, LogOut, User, Settings2, Rows3, Rows4 } from 'lucide-react'
...
export function TopBar({ breadcrumbs, onToggleSidebar }: TopBarProps) {
  ...
  return (
    <>
      ...
      >
        <QkTooltip content="Toggle sidebar">
          <QkButton variant="ghost" size="sm" onClick={onToggleSidebar} aria-label="Toggle sidebar">
            <PanelLeft size={16} />
          </QkButton>
        </QkTooltip>
        ...
        <QkTooltip content="Notifications">
          <QkButton variant="ghost" size="sm" onClick={() => setNotifOpen(true)} aria-label="Notifications" style={{ position: 'relative' }}>
            <Bell size={16} />
            {unread > 0 && (
              <span style={{ position: 'absolute', top: 4, right: 4, width: 7, height: 7, borderRadius: '50%', background: 'var(--qk-danger)' }} />
            )}
          </QkButton>
        </QkTooltip>
